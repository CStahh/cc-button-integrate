const GsTest = artifacts.require("./GsTest")
...
contract('GsTest', ([deployer, user]) => {
    ...
    describe('Deployment', () => {
        ...
        it('Returns the amount of seconds from deployment to wait until minting', async () => {
            ...
            result = await gsTest.allowMintingAfter()
        ...
        it('Returns how many seconds left until minting allowed', async () => {
            let buffer = 2
            let target = Number(milliseconds.toString().slice(0, 3))
            result = await gsTest.getSecondsUntilMinting()
            result = Number(result)
    ...
    describe('Minting', async () => {
        describe('Success', async () => {
            ...
            beforeEach(async () => {
                ...
                result = await gsTest.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') })
            })
            ...
            it('Updates the total supply', async () => {
                result = await gsTest.totalSupply()
                result.toString().should.equal('1')
            })

            it('Returns IPFS URI', async () => {
                result = await gsTest.tokenURI(1)
                result.should.equal(`${IPFS_IMAGE_METADATA_URI}1.json`)
            })

            it('Returns how many a minter owns', async () => {
                result = await gsTest.balanceOf(user)
                result.toString().should.equal('1')
            })

            it('Returns the IDs of minted NFTs', async () => {
                result = await gsTest.walletOfOwner(user)
        ...
        describe('Failure', async () => {
            ...
            beforeEach(async () => {
                ...
                const NFT_MINT_DATE = new Date("April 3, 2022 16:00:00").getTime().toString().slice(0, 10)
            ...
            it('Attempt to mint before mint date', async () => {
                await gsTest.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') }).should.be.rejectedWith(EVM_REVERT)
            })
        })
    })
    ...
    describe('Updating Contract State', async () => {
        describe('Success', async () => {
            ...
            it('Sets the cost', async () => {
                ...
                await gsTest.setCost(cost, { from: deployer })
                result = await gsTest.cost()
            ...
            it('Sets the pause state', async () => {
                ...
                await gsTest.setIsPaused(isPaused, { from: deployer })
                result = await gsTest.isPaused()
            ...
            it('Sets the reveal state', async () => {
                ...
                await gsTest.setIsRevealed(isRevealed, { from: deployer })
                result = await gsTest.isRevealed()
            ...
            it('Sets the max batch mint amount', async () => {
                ...
                await gsTest.setmaxMintAmount(5, { from: deployer })
                result = await gsTest.maxMintAmount()
            ...
            it('Sets the IPFS not revealed URI', async () => {
                ...
                await gsTest.setNotRevealedURI(uri, { from: deployer })
                result = await gsTest.notRevealedUri()
            ...
            it('Sets the base extension', async () => {
                ...
                await gsTest.setBaseExtension('.example', { from: deployer })
                result = await gsTest.baseExtension()
